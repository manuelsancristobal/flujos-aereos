...
from src.config import AMBITOS, PERSPECTIVAS_POR_AMBITO, TIPOS_TRAFICO
...
def run():
    ...
    for ambito in AMBITOS:
        for perspective in PERSPECTIVAS_POR_AMBITO[ambito]:
            for metric in TIPOS_TRAFICO:
                combo = f"{ambito}/{perspective}_{metric}"
                try:
                    logger.info(f"Procesando {combo}...")
                    transformed = build_pipeline(df_jac, airports_chile, airports_global, perspective, metric, ambito)
                    path = load_to_json(transformed, perspective, metric, ambito)
